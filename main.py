...
class NassalMonitor:
    ...
    def _format_time_duration(self, seconds: int) -> str:
        """Форматирует секунды в читаемый формат (как на сайте)"""
            ...
            async with self.session.get(API_URL) as response:
                ...
                for idx, item in enumerate(raw_array):
                    try:
                        ...
                        # ✅ ВАЖНО: Берём точное время игры напрямую из API
    ...
    def _format_streaming_status(self, is_streaming: bool, platforms: List[Dict]) -> str:
        ...
        platform_emojis = {'twitch': '🟣 Twitch', 'youtube': ' YouTube', 'kick': '🟢 Kick',
                           'telegram': '✈️ Telegram', 'vk': '🔵 VK', 'wtv': '📺 WTV', 'vklive': '🔵 VK Live'}
        platform_names = [platform_emojis.get(p.get('platform', '').lower(), p.get('platform', 'Unknown').capitalize()) for p in platforms]
    ...
    def _format_activity_short(self, info: Dict) -> str:
        ...
        if game_title:
            ...
            return f"🎮 Категория: {game_title}{reward_str}" if game_type == 'game' else f"⚡ Категория: {game_title}{reward_str}"
        elif timer_started or (action_kind and action_kind != 'none'):
            return "🎯 Категория: Аукцион" if action_kind == 'auction' else "🎡 Категория: Крутит колесо"
    ...
    async def get_detailed_streamer_info(self, streamer_name: str) -> Optional[str]:
        try:
            ...
            if game_title:
                ...
                # ✅ Показываем точное время из API
    ...
    async def start(self):
        ...
        async def cmd_start(message: types.Message):
            admin_text = "\n🔐 <b>Админ:</b>\n/add_group, /remove_group, /clear_groups, /list_groups\n/test_notify, /my_id, /debug\n/restart_monitor - перезапустить мониторинг" if self.is_admin(message.from_user.id) else ""
