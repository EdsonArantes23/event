...
class NassalMonitor:
    ...
    async def fetch_hltb(self, hltb_game_id: str, title: str) -> str:
        """Получает HLTB время с сайта HowLongToBeat"""
        if not hltb_game_id:
            return ""
        
        cache_key = str(hltb_game_id)
        if cache_key in self.hltb_cache:
            return self.hltb_cache[cache_key]
        
        try:
            url = f"https://howlongtobeat.com/game.php?id={hltb_game_id}"
            headers = {
            ...
            async with self.session.get(url, headers=headers, timeout=10) as resp:
                if resp.status == 200:
                    ...
                    # Ищем HLTB время в HTML (пример: "11 Hours, 7 Minutes")
                    match = re.search(r'(\d+)\s*(?:Hours?|Часов?)\s*,?\s*(\d+)?\s*(?:Minutes?|Минут?)?', html, re.IGNORECASE)
                    if match:
                        hours = int(match.group(1))
                        minutes = int(match.group(2)) if match.group(2) else 0
                        hltb_text = f"{hours} ч {minutes} мин"
                        self.hltb_cache[cache_key] = hltb_text
                        return hltb_text
        except Exception as e:
            ...
            async with self.session.get(API_URL) as response:
                ...
                for idx, item in enumerate(raw_array):
                    try:
                        ...
                        hltb_game_id = auction_result.get('hltbGameId', '')
                        ...
                        # Асинхронно запрашиваем HLTB
                        hltb_time = ""
                        if game_title and hltb_game_id:
                            hltb_time = await self.fetch_hltb(hltb_game_id, game_title)
            ...
            try:
                async with self.session.get(API_URL) as response:
                    ...
                    for idx, item in enumerate(data.get('data', {}).get('array', [])):
                        ...
                        text += f"   hltbGameId: {auction.get('hltbGameId', '-')}\n"
